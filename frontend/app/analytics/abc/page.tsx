...
export default function ABCPage() {
  ...
  const barData = items.slice(0, 20).map(i => ({
        ...
        {summary && (
          <div className="grid grid-cols-3 gap-4">
            {(['A', 'B', 'C'] as const).map(cat => (
              <div key={cat} className={`bg-white rounded-xl shadow-sm p-5 border-l-4 cursor-pointer transition ${
                filterCat === cat ? 'ring-2 ring-offset-1' : ''
              }`}
                style={{ borderLeftColor: CAT_COLORS[cat] }}
                onClick={() => setFilterCat(filterCat === cat ? 'all' : cat)}
              >
                <div className="flex items-center justify-between mb-2">
                  <span className={`text-xl font-bold px-3 py-0.5 rounded-full ${CAT_BG[cat]}`}>
                    {cat}
                  </span>
                  <span className="text-2xl font-bold text-gray-800">
                    {summary[`${cat.toLowerCase() as 'a'|'b'|'c'}_pct`]}%
                  </span>
                </div>
                <p className="text-sm text-gray-500">
                  {summary[`${cat.toLowerCase() as 'a'|'b'|'c'}_count`]} позиций
                </p>
                <p className="text-sm font-semibold text-gray-700 mt-1">
                  {fmt(summary[`${cat.toLowerCase() as 'a'|'b'|'c'}_volume`])} кг
                </p>
                ...
                {loading ? (
                  ...
                  <tr key={item.rank} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2.5 text-center">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${CAT_BG[item.category]}`}>
                        {item.category}
                      </span>
                    </td>
                  </tr>
